...
public interface GossipConfiguration {
    ...
    /**
     * Returns the socket port where the gossiper will listen for incoming gossips. Returning 0 serves as an indication
     * for the OS to bind to any free port.
     * 
    ...
    /**
     * Returns whether gossiper is enabled on this system or not. In practice to indicate that gossiper is enabled one
     * only needs to specify at least one seed node.
     * 
     * @return <code>true</code> if gossiper is enabled on this system or <code>false</code>.
